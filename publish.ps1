...
$session = $null
$response = Invoke-WebRequest -Uri "https://www.tianliaos.com/dong721/login" -Method POST -Body $body -SessionVariable session
$cookies = $session.Cookies.GetCookies("https://www.tianliaos.com")
$token = ($cookies | Where-Object { $_.Name -eq "token" }).Value
...
> 注意：请将创空间设为 **非公开**，否则他人可能操纵你的 CoPaw。
...
"@
    ...
    $postBody = @{
        title = "CoPaw：3分钟部署你的AI个人助理，本地云端都能跑！"
        content = $content
        category = "tech"
        tags = "AI,开源,工具"
        status = "published"
    ...
    $postResponse = Invoke-WebRequest -Uri "https://www.tianliaos.com/dong721/post/new" -Method POST -Body $postBody -WebSession $session
    Write-Host "Post response: $($postResponse.StatusCode)"
} else {
